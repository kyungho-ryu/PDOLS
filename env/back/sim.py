...
class Simul:
            ...
            for itNext in self.ues:
                ...
                if dist < it.maxCellRadius and it.id != itNext.id:
                    ...
                    if pLink == None:
                        ...
                        if it.id != 0:  # small cell access links
    ...
    def checkPathCapa(self, p):
        ...
        for idx in range(len(p)-1):
            if (p[idx], p[idx+1]) in self.anDownlinks: #access links
                ...
                #print("r", p[idx+1], pLink.n[0].allocPrb, reqRbs, pLink.maxPRB)
                ...
                if (pLink.assNumFlows +1) * self.ueDataRate > pLink.maxCapa:
                    bCapable = False
                    minRate = pLink.maxCapa - (pLink.assNumFlows * self.ueDataRate)
    ...
    def placeUserFlowOnLinks(self, p, bFind, bRate):
        for idx in range(len(p)-1):
            if (p[idx], p[idx+1]) in self.anDownlinks: #access links
                pLink = self.anDownlinks[(p[idx], p[idx+1])]
                if bFind:
    ...
    def updateUserFlowPath(self):
        ...
        for u in self.ues:
            ...
            if u not in self.hotspotUe:
                ...
                scellEdges = {**self.bhlinks}

                for k, v in self.scellAcessLink.items() :
                    if k[1] == u.id : scellEdges[k] = v
                path = self.nxTopo.getPathViaSeNBs(u.id, 0, list(scellEdges.keys()))
                path.reverse(); bFind, bRate = self.checkPathCapa(path);
                self.currentUePath[u.id] = (path, bFind)
                self.placeUserFlowOnLinks(path, bFind, bRate)
                ...
                if bFind :
                    ...
                    if bRate == 0 :  blocked +=1



        self.currentUeInLink = {}
        for k, l in self.bhlinks.items():
            self.currentUeInLink[k] = l.activeLink

        self.blocking = blocked / len(self.ues)

    def genNxTopo(self):
        self.edges = {**self.bhlinks, **self.scellAcessLink}
        self.nxTopo = Topology(self.numNodes, self.numSmall, list(self.edges.keys()))
    ...
    def calLinkCapacity(self):
    	...
    	for lit in self.anDownlinks.values():
    ...
    def getLinksLoad(self):
        ...
        for b in self.bhlinks.values():
            linkWorkload.append((self.ueDataRate * b.assNumFlows)/b.maxCapa)
        for a in self.scellAcessLink.values():
            linkWorkload.append(a.n[0].allocPrb/a.maxPRB)
    ...
    def calEnergyConsumption(self):
        ...
        for n in self.enbs:
            if n.id == 0: #macro6
                if sum([bh.assNumFlows for bh in n.bhLinks]) ==0:
                    macroTotalPower = 0
                    eNBPower = 0
                else:
                    ...
                    macroBhPower = 0
                    for bh in n.bhLinks:
                        macroBhPower += 3.9 + 0.224*(bh.assNumFlows*self.ueDataRate/bh.maxCapa)
                ...
                if sum([bh.assNumFlows for bh in n.bhLinks]) ==0:
                    ...
                    scellBhPower = 0
                    for bh in n.bhLinks:
                        scellBhPower += 3.9 + 0.224* min((bh.assNumFlows*self.ueDataRate/bh.maxCapa), 1)
        ...
        for n in self.enbs:
            if n.id == 0: #macro6
                ...
                if sum([bh.assNumFlows for bh in n.bhLinks]) ==0:
                    ...
                    scellBhPower = 0
                    for bh in n.bhLinks:
						#print("bh", bh.assNumFlows,3.9 +0.224* bh.assNumFlows*self.ueDataRate/bh.maxCapa)
                        scellBhPower += 3.9 + 0.224 * min((bh.assNumFlows*self.ueDataRate/bh.maxCapa), 1)
    ...
    def getActiveSenbs(self):
        ...
        for n in self.enbs:
            if n.id != 0:
                if sum([bh.activeLink for bh in n.bhLinks]) != 0 :
                    usedSenbs +=1
