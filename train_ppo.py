...
def start(env, La, learning_rate) :
    env.setLa(La)
    ...
    file_name = getFileName("PPO(test)", p.WEIGHT_INITIALIZATION, p.WEIGHT_MU, p.LOSS_TYPE, learning_rate, learning_rate, p.GAMMA, p.GAE_LAMBDA, p.BATCH_SIZE, p.REPLAY_SIZE,
    ...
    opt_crt = optim.Adam(net_crt.parameters(), lr=learning_rate)
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        for step_idx, exp in enumerate(exp_source):
            ...
            print('{}-state : {}'.format(step_idx, env.sim.getLinksLoad()))
            print('{}-blocking : {}'.format(step_idx, env.getBlockingPro()))
            print('{}-blocked node : {}'.format(step_idx, env.sim.blockedNode))
            ...
            if len(tracker.total_rewards) >= 500 :
                return
...
#[0.8,0.2],
    La = [[0.4,0.3], [0.8,0.2], [0.5,0.5]]
    # learning_Rate = [0.0003, 0.0001, 0.00005, 0.0001]
    # La = [[0.6,0.4]]
    for i in La :
            start(env, i, 0.0001)
    # for i in La :
